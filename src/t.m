...
if max(x(1, 1:2000)) > 0.7
	x(1, 1:2700) = x(1, 31001-2700:31000);
end
x = x ./ max(x);
%幅度归一化到[-1,1]  
x = double(x);  
...
amp1 = 0.2 * 59;%初始短时能量高门限  
amp2 = 0.23 * amp1;%初始短时能量低门限  
zcr1 = 0.2 * 29;%初始短时过零率高门限  
...
%计算过零率  
tmp1  = enframe(x(1:end-1), hamming(FrameLen,'periodic'), FrameInc);%分帧，所得矩阵为fix（（x(1:end-1)-FrameLen+FrameInc）/FrameInc）*FrameLen  
tmp2  = enframe(x(2:end)  , hamming(FrameLen,'periodic'), FrameInc);%分帧，所得矩阵为fix（（x(2:end)-FrameLen+FrameInc）/FrameInc）*FrameLen  
...
amp = sum(abs(enframe(x, hamming(FrameLen,'periodic'), FrameInc)), 2);  
